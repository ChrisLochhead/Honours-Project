...
public class AIController : Agent {
    ...
    //For preventing dying on walls in the study
    ...
    private void Start()
    {
        ...
        if (gameInfo)
        {
            ...
            gameObject.GetComponent<DemonstrationRecorder>().paths = GameObject.Find("SessionManager").GetComponent<PathContainer>();
        ...
        //If in the study, a world positon needs to be found
        //as the prefab world position cannot be assigned automatically 
        //Also assign the agent it is attacking in this part
        if (!worldPosition)
        {
            if (GameObject.Find("PathGrid"))
            {
                worldPosition = GameObject.Find("PathGrid");
            }
        }
    }
    ...
    public override void AgentAction(float[] vectorAction, string textAction)
    {
         ...
         */
        if (brain.brainParameters.vectorActionSpaceType == SpaceType.discrete && controller.isAlive)
        {
    ...
    private void ResetAgentModel()
    {
        ...
        for (int i = 0; i < weaponManager.clipSize.Length; i++)
        {
            ...
            weaponManager.currentMaxAmmo[i] = weaponManager.maxAmmo[i];
        }
